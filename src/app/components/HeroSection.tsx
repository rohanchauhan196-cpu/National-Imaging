  ...
  return (
    <section id="home" className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="max-w-3xl text-left mt-16 md:mt-0 lg:mt-12">
          {/* Badge */}
          <motion.div
          ...
          >
            <span className="block text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-bold text-white drop-shadow-2xl">
              {current.title[0]}
            </span>
            <span className="block text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-bold drop-shadow-2xl" style={{ color: '#ed3237' }}>
              {current.title[1]}
            </span>
          </motion.h1>
          ...
          <motion.p
            ...
            className="text-sm sm:text-base md:text-lg text-white/90 mb-6 md:mb-10 max-w-2xl drop-shadow-lg"
          >
            {current.subtitle}
          </motion.p>
          ...
          >
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
              className="bg-secondary text-white px-4 py-2 sm:px-6 sm:py-3 md:px-8 md:py-4 rounded-full text-sm sm:text-base md:text-lg font-semibold shadow-2xl hover:bg-secondary/90 transition-all duration-300 flex items-center gap-2"
            >
            ...
            <motion.button
              whileHover={{ scale: 1.08 }}
              whileTap={{ scale: 0.95 }}
              className="bg-white/20 backdrop-blur-md text-white border-2 border-white px-4 py-2 sm:px-6 sm:py-3 md:px-8 md:py-4 rounded-full text-sm sm:text-base md:text-lg font-semibold hover:bg-white/30 transition-all duration-300"
            >
              View Services
            </motion.button>
          </motion.div>
          ...
          <motion.div
            ...
            className="grid grid-cols-3 gap-2 sm:gap-4 md:gap-8 max-w-xl"
          >
            <StatItem icon={<Award className="w-5 h-5 sm:w-6 sm:h-6 md:w-10 md:h-10" />} value="NABL" label="Certified" />
            <StatItem icon={<Users className="w-5 h-5 sm:w-6 sm:h-6 md:w-10 md:h-10" />} value="50k+" label="Patients Served" />
            <StatItem icon={<Clock className="w-5 h-5 sm:w-6 sm:h-6 md:w-10 md:h-10" />} value="24/7" label="Support" />
          </motion.div>
        </div>
      </div>
    </section>
...
const StatItem = ({ icon, value, label }: { icon: React.ReactNode; value: string; label: string }) => (
  ...
  >
    ...
    <p className="text-lg sm:text-xl md:text-3xl font-bold drop-shadow-lg">{value}</p>
